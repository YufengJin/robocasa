...
if [ -f /workspace/pyproject.toml ] || [ -f /workspace/setup.py ]; then
  if [ ! -d "${ROBOCASA_ASSETS}/textures" ] || [ -z "$(ls -A "${ROBOCASA_ASSETS}/textures" 2>/dev/null)" ]; then
    ...
    yes | python /workspace/robocasa/scripts/download_kitchen_assets.py
  else
...
if ([ -f /workspace/pyproject.toml ] || [ -f /workspace/setup.py ]) \
   && ! python -c "import robocasa.macros_private" 2>/dev/null; then
  echo "Setting up private macros..."
  python /workspace/robocasa/scripts/setup_macros.py
fi
